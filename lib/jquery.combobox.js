(function($) {
	$.widget("custom.combobox", {
		_create : function() {
			...
			this.element.hide();
			this._createIconImage();
			this._createAutocomplete();
			this._createShowAllButton();
		},

		_createIconImage : function() {
			var selected = this.element.children(":selected").attr('icon');
			var defval = this.element.children("option").attr('icon');
			var icon;
			...
			this.icon = $("<img />").appendTo(this.wrapper).attr('src', icon).addClass("custom-combobox-icon ui-widget");
		},
		...
		_createAutocomplete : function() {
			...
			this.input = $("<input />").appendTo(this.wrapper).val(value).attr("title", "").addClass("custom-combobox-input ui-widget ui-widget-content ui-state-default ui-corner-left").autocomplete({
				...
				focus : $.proxy(function(e, ui) {
					if (ui.item) {
						var icon = $(ui.item.option).attr('icon');
				...
				var icon = $(this.element.children(":selected")[0]).attr('icon');
				if (icon !== undefined && this.icon !== undefined ) {
					this.icon.attr('src', icon);
				}
			}, this));
