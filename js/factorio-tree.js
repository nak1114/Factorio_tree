...
Factorio.tree.prototype = {
    ...
    prenode : function(req_spd, id, parent_node, ary, root_id) {
        var h = this;
        ...
        $.each(tgt.ingredients, function() {
            h.prenode(req_spd * this[1], this[0], node, ary, root_id);
        });
        return node;
    },
    preroot : function(req_spd, id, type, source_id, source_num) {
        ...
        if (h.subtable[type] === undefined) {
            ...
            hash.list = ary;
            hash.status = 'preroot';
            var node = h.prenode(1.0, id, -1, ary, type);
        }
        if (source_num >= 0) {
        ...
        $.each(h.subtable,function(key,val){
            ...
            if (val.status == 'preroot') {
                ...
                $.each(val.list, function(num) {
                    ...
                    var rows = $("<tr>").attr("data-tt-id", i.node)
                    .attr('type',key).attr('num',num);
                    if (i.parent_node >= 0) {
    ...
    root : function(req_spd, id) {
        ...
        var type ='main-' + h.cur;
        //var rows = $("<tr>").appendTo(this.table);
        //$("<td>").appendTo(rows).attr('colspan', 5).text("test");
